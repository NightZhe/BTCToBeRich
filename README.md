...
src/analyze.js       訊號品質檢討工具：逐筆量化「距低點多遠、後續漲跌、是否破低」
```
...
```bash
node src/analyze.js 2026-06-17              # 起始日到現在
node src/analyze.js 2026-07-10 2026-07-17   # 指定區間（結束日不含當天）
```

用與實盤一致的邏輯重現區間內每筆訊號，逐筆量化買點品質：進場點距 V 型低點
幾分鐘/幾 %、V 型回撤比例、30分/1小時/2小時/4小時/24小時後續報酬、24 小時內
最大不利/有利波動、是否跌破 V 型低點（= 反轉失敗），最後附整體統計。
詳細判讀方法見 `docs/signal-review-20260717.md`。

## 網頁儀表板

`src/monitor.js` 啟動時會同時起一個 Node 內建 http server（不裝任何套件），
提供手機/電腦瀏覽器看的儀表板，方便確認「監控有沒有在動、最近檢查了什麼、
觸發過什麼訊號」。歷程存在記憶體：檢查紀錄保留 7 天、訊號保留 30 天。
兩者都由 K 線數據決定、可重算，所以啟動時會自動抓過去 30 天 K 線回補
（訊號 30 天、檢查紀錄 7 天，只填清單不發通知），重啟也不會丟。
